...
function handleVote($redis, $data, $userId)
{
    ...
    $bookId = preventXSS($data['vote_id']);
    ...
    try {
        ...
        // 批量更新各種投票記錄
        $redis->zadd($allUserVotesKey, $voteTimestamp, "{$today}:{$bookId}:{$voteJson}");
        $redis->sadd($userDailyVotesKey, $bookId);
        ...
        // 使用syncBookVotes函數獲取最新投票數並同步到排行榜
        $votes = syncBookVotes($redis, $bookId);

        // 獲取用戶今日已投票的書籍清單和剩餘票數
        $todayVotedBooks = $redis->smembers($userDailyVotesKey);
        $remainingVotes = $maxDailyVotes - count($todayVotedBooks);
...
function checkVoteStatus($redis, $data, $userId)
{
    ...
    try {
        ...
        $userDailyVoteCount = count($todayVotedBooks);
        ...
        sendJsonResponse(true, "檢查完成", [
            ...
            'book_title' => $bookTitle,
            'votes' => $votes,
            'hasVotedForBookToday' => $hasVotedToday,
...
function batchCheckVoteStatus($redis, $userId)
{
    ...
    try {
        ...
        // 如果有投票的書籍，獲取書籍詳情
        $booksDetails = [];
        if (!empty($todayVotedBooks)) {
            // 建立批量查詢管道獲取書籍詳情
            $bookPipe = $redis->pipeline();
            foreach ($todayVotedBooks as $bookId) {
                $bookPipe->hgetall("book:{$bookId}");
            }
            $bookResults = $bookPipe->execute();

            // 處理書籍詳情結果
            foreach ($todayVotedBooks as $index => $bookId) {
                $bookData = $bookResults[$index];
                if (!empty($bookData)) {
                    // 只提取需要的字段
                    $booksDetails[$bookId] = [
                        'id' => $bookId,
                        'title' => $bookData['title'] ?? '未知書籍',
                        'votes' => (int)($bookData['votes'] ?? 0)
                    ];
                }
            }
        }
        ...
        sendJsonResponse(true, "批量檢查完成", [
            'user_id' => $userId,
            'today_voted_books' => $todayVotedBooks,
            'books_details' => $booksDetails, // 添加書籍詳情
            'daily_votes_used' => $userDailyVoteCount,
            'daily_votes_remaining' => $remainingVotes,
            'max_daily_votes' => $maxDailyVotes,
...
function getUserVoteHistory($redis, $userId)
{
    ...
    try {
        ...
        if (!empty($uniqueBookIds)) {
            ...
            // 處理書籍詳情結果
            foreach ($uniqueBookIds as $index => $bookId) {
                $bookData = $bookResults[$index];
                if (!empty($bookData)) {
                    // 只保存需要的書籍信息
                    $bookDetails[$bookId] = [
        ...
        foreach ($voteDetails as &$detail) {
            ...
            if (isset($bookDetails[$bookId])) {
                $detail['book_title'] = $bookDetails[$bookId]['title'];
                $detail['book_votes'] = $bookDetails[$bookId]['votes'];
                $detail['book_author'] = $bookDetails[$bookId]['author'];
                $detail['book_publisher'] = $bookDetails[$bookId]['publisher'];
            }
        }
        unset($detail); // 釋放引用
        ...
        // 獲取當日投票的書籍詳情
        $todayVotedBooksDetails = [];
        if (!empty($todayVotedBooks)) {
            foreach ($todayVotedBooks as $bookId) {
                if (isset($bookDetails[$bookId])) {
                    $todayVotedBooksDetails[$bookId] = $bookDetails[$bookId];
                }
            }
        }
        ...
        sendJsonResponse(true, "獲取用戶投票歷史成功", [
            ...
            'max_daily_votes' => $maxDailyVotes,
            'today_voted_books' => $todayVotedBooks,
            'today_voted_books_details' => $todayVotedBooksDetails, // 添加書籍詳情
